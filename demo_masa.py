...
def inference(input_data, **params):
    ...
    # print(hint.shape)
    ref_img = np.ones((H, W, 3)) * 127.5
    masked_img = ((ref_img.astype(np.float32) / 127.5) - 1.0) * (1 - flat_hint)
    masked_img = np.transpose(masked_img, (2, 0, 1))
    masked_img = torch.from_numpy(masked_img.copy()).float().cuda()
    encoder_posterior = model.encode_first_stage(masked_img[None, ...])
    flat_masked_x = model.get_first_stage_encoding(encoder_posterior).detach()
    info["flat_masked_x"] = torch.cat(
        [flat_masked_x for _ in range(params["image_count"])], dim=0
    )
